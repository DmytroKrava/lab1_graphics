...
function Model(name) {
    ...
    this.DrawLine = function () {
        ...
        webGLCont.vertexAttribPointer(shaderProgram.iAttribVertex, 3, webGLCont.FLOAT, false, 0, 0);
        webGLCont.enableVertexAttribArray(shaderProgram.iAttribVertex);
        webGLCont.drawArrays(webGLCont.LINE_STRIP, 0, this.count)
    }
}
...
function draw() {
    ...
    let px = 10 * Math.cos(Date.now() * 0.001),
        py = 10 * Math.sin(Date.now() * 0.001),
        pz = parseFloat(document.getElementById('positionOfLightZ').value)
    ...
    webGLCont.uniformMatrix4fv(shaderProgram.iModelViewProjectionMatrix, false, m4.multiply(modelViewProjection, m4.translation(
        px, py, pz)));
    sphere.Draw()
    line.BufferData([0, 0, 0, -px, -py, -pz], [0, 0, 0, 1, 1, 1])
    line.DrawLine()
}
...
function CreateSphereData() {
    ...
    for (let u = 0; u < Math.PI * 2; u += 0.1) {
        for (let t = 0;  t < Math.PI;    t += 0.1) {
            ...
            vertexes.push(v.x, v.y, v.z);
            normals.push(v.x, v.y, v.z);
            vertexes.push(w.x, w.y, w.z);
            normals.push(w.x, w.y, w.z);
            vertexes.push(wv.x, wv.y, wv.z);
            normals.push(wv.x, wv.y, wv.z);
            vertexes.push(wv.x, wv.y, wv.z);
            normals.push(wv.x, wv.y, wv.z);
            vertexes.push(w.x, w.y, w.z);
            normals.push(w.x, w.y, w.z);
            vertexes.push(ww.x, ww.y, ww.z);
            normals.push(ww.x, ww.y, ww.z);
